<template>
	<div class="commonDct" ref="getHeight">
		...
		<div class="floor2">
			<BaseTable pageSize="15" :handleCurrentChange="handleCurrentChange" :handleSizeChange="handleSizeChange" ref="BaseTable" @sortBy="queryTableData" @selectedTableData="Left1_selectedTableData" :currentPage="currentPage" :callBack="cbFunc" :tableHead="tableHead" :tableData.sync="tableData" :tableHeight="tableData.tableHeight"></BaseTable>
		</div>
	</div>
</template>
		...
		methods: {
			...
			queryTableData(column) {
				...
				if(column) {
					...
					if(this.$refs.BaseTable.Me_tableData.DCT_SORT) {
						params.sortBy = " ORDER BY " + this.$refs.BaseTable.Me_tableData.DCT_SORT
					}
				}
